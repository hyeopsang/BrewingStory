...
export function getOpenStatusFromDescriptions(weekdayDescriptions: string[]): OpenStatus {
  const now = new Date();
  ...
  if (!isOpen) {
    ...
    const { openMinutes: tomorrowOpen, closeMinutes: tomorrowClose } = extractOpenCloseMinutes(tomorrowDesc);
...
function extractOpenCloseMinutes(desc: string): { openMinutes: number; closeMinutes: number } {
  const timeMatch = desc.match(/(오전|오후)\s?(\d+):(\d+)\s?~\s?(오전|오후)\s?(\d+):(\d+)/);

  if (!timeMatch) {
    return { openMinutes: 0, closeMinutes: 0 };
  }

  const [, openMeridiem, openHour, openMin, closeMeridiem, closeHour, closeMin] = timeMatch;

  const open = to24Hour(parseInt(openHour), openMeridiem);
  const close = to24Hour(parseInt(closeHour), closeMeridiem);

  return {
    openMinutes: open * 60 + parseInt(openMin),
    closeMinutes: close * 60 + parseInt(closeMin),
  };
}
